...
public class GuiManager : MonoBehaviour
{
    ...
    public T Find<T>() where T : GuiBase
    {
        ...
        {
            if(_guiEntityPools[i] != null && _guiEntityPools[i].gameObject.name == typeof(T).ToString())
            {
    ...
    public T Show<T>(bool pShow) where T : GuiBase
    {
        GuiBase guiBase = Find< T>();
        guiBase.Show(pShow);
        return guiBase as T;
    }
    public T Show<T>(ELayerType pLayer, bool pShow, JSONNode pParams = null) where T : GuiBase
    {
    ...
    { 
        if (pShow)
        {
            ...
            for ( int i = 0; i < _guiEntityPools.Count; ++i )
            {
                ...
                if (guiBase != null && guiTypeName == guiBase.GetType().ToString())
                {
    ...
    public void Set_Position_WorldPoint(ELayerType pLayer, Transform obj_trans, Vector3 world_target)
    {
        Transform parentTrans = _TransBack;
        switch (pLayer)
        {
            case ELayerType.Back:
                {
                    parentTrans = _TransBack;
                }
                break;

            case ELayerType.Front:
                {
                    parentTrans = _TransFront;
                }
                break;
            case ELayerType.Cache:
                {
                    parentTrans = _TransCache;
                }
                break;
        }

        //타겟의 포지션을 월드좌표에서 ViewPort좌표로 변환하고 다시 ViewPort좌표를 NGUI월드좌표로 변환합니다.
        Vector3 pos = parentTrans.GetComponent<Camera>().ViewportToWorldPoint(Camera.main.WorldToViewportPoint(world_target));

        obj_trans.position = new Vector3(pos.x, pos.y, parentTrans.position.z);
    }

    public GameObject SetSpawnGui_WorldPoint(ELayerType pLayer, GameObject prefab, Vector3 world_target)
    {
        Transform parentTrans = _TransBack;
        switch (pLayer)
        {
        ...
        GameObject obj = GuiSpawnerPool.Spawn(prefab, new Vector3(pos.x, pos.y, parentTrans.position.z ) , Quaternion.identity);
