...
public class BezierCurve : MonoBehaviour {
    ...
    public LineRenderer m_LineRenderer_dummy;
	...
	void OnDrawGizmos () {
		...
		if(points.Length > 1){
			for(int i = 0; i < points.Length - 1; i++){
				DrawCurve(points[i], points[i+1], resolution, points[i].m_LineRenderer);
			}
			
			if (close) DrawCurve(points[points.Length - 1], points[0], resolution, points[points.Length - 1].m_LineRenderer);
		}
	}
	...
	public BezierPoint AddPointAt(Vector3 position)
	{
        ...
        GameObject pointObject = Instantiate(m_LineRenderer_dummy.gameObject); //  new GameObject("Point "+pointCount);
        pointObject.name = string.Format("Point {0}", pointCount);
	...
	/// </param>
	public static void DrawCurve(BezierPoint p1, BezierPoint p2, int resolution, LineRenderer line_renderer)
	{
        ...
        {
            line_renderer.SetVertexCount(limit);
            line_renderer.SetPosition(0, lastPoint);
            for (int i = 1; i < limit; i++)
            {
                currentPoint = GetPoint(p1, p2, i / _res);
                line_renderer.SetPosition(i, currentPoint);
                // lastPoint = currentPoint;
            }
        }
